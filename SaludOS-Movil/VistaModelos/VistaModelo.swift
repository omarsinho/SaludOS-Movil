...
class VistaModelo: ObservableObject {
    ...
    var countsDIA = [Double: Int]()
    var countsSYS = [Double: Int]()
    var countsPulso = [Double: Int]()
    
    @Published var modaDIA = 1.1
    @Published var modaSYS = 1.1
    @Published var modaPulso = 1.1
    
    @Published var vecesDIA = 1
    @Published var vecesSYS = 1
    @Published var vecesPulso = 1
    
    @Published var medianaDIA = 1.1
    @Published var medianaSYS = 1.1
    @Published var medianaPulso = 1.1
                ...
                if let snapshot = snapshot {
                    ...
                    DispatchQueue.main.async {
                        ...
                        for doc in snapshot.documents {
                            ...
                            self.arregloDIA.append(doc["presionDiastolica"] as? Double ?? 0)
                            self.arregloSYS.append(doc["presionSistolica"] as? Double ?? 0)
                            self.arregloPulso.append(doc["pulso"] as? Double ?? 0)
                            self.cantidadDatos += 1
                            self.sumDIA += doc["presionDiastolica"] as? Double ?? 0
                            self.sumSYS += doc["presionSistolica"] as? Double ?? 0
                            self.sumPulso += doc["pulso"] as? Double ?? 0
                        }
